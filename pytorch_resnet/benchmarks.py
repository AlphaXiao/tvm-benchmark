...
def benchmark(model, csv_file, input_fn=genImage, iters=100, warmup=10):
    with torch.no_grad():
        ...
        with autotvm.apply_history_best(fn):
            ...
            print("Done benchmarking TVM, which compiled {:.2f}% of compute".format(
            ...
            if csv_file:
                ...
                with open(csv_file, 'a' if exists else 'w') as f:
                    ...
                    f.write("{},{}\n".format(
                        int(time.time()), iters / tvm_time))
        print("JIT: {} iter/s\nTVM: {} iter/s".format(iters /
                                                      jit_time, iters / tvm_time))


def run_benchmark(csv_file):
    # model = test_models.resnet34(True)
    model = test_models.resnet18(True)
    model.eval()
